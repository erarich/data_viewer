...
def main():
    ...
    components.html(""" <iframe title="Report Section" width="600" height="373.5" src="https://app.powerbi.com/view?r=eyJrIjoiZDc0ZWNkZGItNmUwYy00MzRlLWJiOTQtZjg3MThmYjA5M2RlIiwidCI6ImU4MDQ3OWI0LTI2ZWQtNGQ2OS1hZWZhLTgwMTk0MDliMDQ1MiJ9" frameborder="0" allowFullScreen="true"></iframe>""", height=600)
    ...
    selected_separator = st.selectbox(
        "Select a separator: ", (separators), key="sb_three")

    uploaded_file = st.file_uploader("Choose a .csv file", type=[
        'csv'], accept_multiple_files=False)

    if uploaded_file is not None:
        dataframe = convert_to_dataframe(uploaded_file, selected_separator)
